...
# Step 3: optionally render the final config to stdout and stop.
#
# This mode is used by tests and by humans who want to inspect the generated
# config without installing anything into /etc or restarting Mosquitto.
...
# Step 5: install Mosquitto if the host does not already provide it.
...
# Step 7: install the rendered config into Mosquitto's system config directory.
...
# Step 9: enable and restart Mosquitto so ESP32 nodes can connect to MQTT.
